...
import com.tapestry.stackoverflowclone.pages.Index;
import com.tapestry.stackoverflowclone.pages.Login;
...
public class PageProtectionFilter implements ComponentRequestFilter {
    ...
    private boolean isAuthenticated() throws IOException {
        ...
        if (user != null) {
            if (user.getUserRole() == Role.Admin || user.getUserRole() == Role.User) {
                return true;
            }
        }
        return false;
